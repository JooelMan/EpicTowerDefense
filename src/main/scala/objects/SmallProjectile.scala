...
class SmallProjectile(x: Double, y: Double, target: Vector2D) extends Projectile(x, y, target) {

  val projectileRadius = 3

  var speed = target - location
  while (speed.length > 12) {
    speed = speed / 1.4
  }
  ...
  def update() = {
    if (location.x == target.x && location.y == target.y
          || (location - startLocation).length > Tower.greyShootRange
